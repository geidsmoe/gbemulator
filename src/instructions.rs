...
pub fn gameboy_doctor_cpu_log(cpu: &CPU) {
  ...
  println!("A:{:02X} F:{:02X} B:{:02X} C:{:02X} D:{:02X} E:{:02X} H:{:02X} L:{:02X} SP:{:04X} PC:{:04X} PCMEM:{:02X},{:02X},{:02X},{:02X} DPAD: {:02X} BUTTONS: {:02X}",
              ...
              cpu.read(cpu.pc.wrapping_add(3) as usize), cpu.dpad, cpu.buttons);
}
...
pub fn execute_opcode(instruction_set: &InstructionSet, cpu: &mut CPU, show_debug_messages: bool) -> u32 {
  ...
  match opcode  {
    ...
    0x22 => {
      ...
      cpu.registers.set_hl(hl_value.wrapping_add(1));
    }
    0x32 => {
      let hl_value = cpu.registers.get_hl();
      cpu.write(hl_value as usize, cpu.registers.a);
      cpu.registers.set_hl(hl_value.wrapping_sub(1));
    }
    0x06 => {
